...
void StateMachineUpdate(void){
	...
	switch(State) {
		case Init: {
			State = Init;
			//переход заготовки с ленты CB2 на ленту СВ3, если заготовка на ленте СВ2 или на столе RT2 
			if (s[7] || s[10]) State = RT2_init;
			//переход заготовки с ленты СВ3 на ленту СВ1, если заготовка на ТТ или на ленте СВ3
			if (s[2] || s[11]) State = TT_init;
			//переход заготовки с ленты CB1 на ленту СВ2, если заготовка на ленте СВ1 или на столе RT1
			if (s[3] || s[6]) State = RT1_init;		
			// работа фрезерного станка , если заготовка на ленте СВ2
			if (s[7]) State = MM_init;					
			break;
		}
		
		//перемещение груза с ленты СВ2 на ленту СВ3, при помощи стола RT2
		case RT2_init: { 
			State = Init;
			// вращение RT2 в горизонтально, если заготовка на ленте СВ2 и стол RT2 не стоит горизонтально (поворотный стол 2 не на одной линии с конвейерной лентой 2)  
			if (s[7] && !s[8]) State = RT2_rotate_l;
			// перемещение груза с ленты СВ2 на стол RT2, если поворотный стол RT2 на одной линии с конвейерной лентой СВ2 и заготовка не на ленте СВ2
			if (s[8] && !s[10]) State = RT2_download; 
			// вращение RT2 в вертикально, если заготовка на поворотном столе RT2 и стол RT2 не стоит вертикально (поворотный стол 2 не на одной линии с конвейерной лентой 3)			
			if (s[10] && !s[9]) State = RT2_rotate_r;	
			// перемещение груза с стола RT2 на ленту СВ3, если поворотный стол RT2 на одной линии с конвейерной лентой СВ3(вертикально) и заготовка не на ленте СВ3
			if (s[9] && !s[11]) State = RT2_upload;	
			// нахождение груза на ленте СВ3, если поворотный стол RT2 на одной линии с конвейерной лентой СВ3(вертикально) и заготовка на ленте СВ3
		...
		// нахождение груза на ленте СВ3 и возвращение стола RT2 в горизонтальное положение
		case RT2_to_wait: {	
			SetY(9);//поворотный стол RT2 - повернуть на конвейер СВ2(гор)
			if (s[9] && s[11]) State = RT2_to_wait;
			else State = RT2_init;
		...
		//переход заготовки с ленты СВ3 на ленту СВ1, при помощи транспортного стола TT
		case TT_init: {	
			State = Init;
